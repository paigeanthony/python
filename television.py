...
class Television:
    ...
    def mute(self) -> None:
        '''Sets the mute status of the t.v.; set to the minimum volume when muted;
        aset to previous value when unmuted'''
        if self.__status:
    ...
    def channel_up(self) -> None:
        '''Increases channel number by 1; if at max channel, wraps to min channel'''
        if self.__status:
    ...
    def channel_down(self) -> None:
        '''Decreases channel number by 1; if at min channel, wraps to max channel'''
        if self.__status:
    ...
    def volume_up(self) -> None:
        '''Increases the volume by 1; unmutes if unmuted; volume doesn't exceed max volume'''
        if self.__status:
            if self.__muted:
                self.__muted = False
                self.__volume = self.__pre_volume
    ...
    def volume_down(self) -> None:
        '''Decreases the volume by 1; unmutes if unmuted; volume doesn't fall below min volume'''
        if self.__status:
            if self.__muted:
                self.__muted = False
                self.__volume = self.__pre_volume
